...
EPUBJS.reader.search.request = function(q, callback) {
	var fetch = $.ajax({
		dataType : "json",
		url : EPUBJS.reader.search.SERVER + "/search?q="
...
EPUBJS.reader.plugins.SearchController = function(Book) {
	...
	var $searchBox = $("#searchBox"), $searchResults = $("#searchResults"), $searchView = $("#searchView"), iframeDoc;
	...
	var query = function() {
		...
		EPUBJS.reader.search.request(q, function(data) {
			...
			$(iframeDoc).find('body').highlight(q, {
				element : 'span'
			});

			results.forEach(function(result) {
				var $li = $("<li></li>");
				var $item = $("<a href='" + result.href + "' data-cfi='"
						+ result.cfi + "'><span>" + result.title + "</span><p>"
						+ result.highlight + "</p></a>");

				$item.on("click", function(e) {
					var $this = $(this), cfi = $this.data("cfi");
